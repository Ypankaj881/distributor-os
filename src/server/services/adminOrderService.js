...
import { startOfDay, addDays } from "../../lib/dates.js";
...
function listFilter(companyId, { status, q, customerId, from, to }, timeZone) {
...
export async function listAdminOrders(companyId, query, { timeZone = "Asia/Kolkata" } = {}) {
  ...
  const [total, orders] = await Promise.all([
    ...
    Order.find(filter)
      .select("orderNumber status paymentStatus grandTotal createdAt customerId customerSnapshot.shopName customerSnapshot.customerCode items.orderedQty source")
